...
| 可选 **searchRegion** | 屏幕坐标（Spotter 现有约定） |
...
screen.find / findInWindow
  ...
  → spotter-plugin-match-ncc   （唯一 MatchPlugin 实现）
```
...
- 模板上限：512×512
- 扩展点：`spotter-base::MatchPlugin` trait 可挂新 backend
...
Spotter 场景仍是：**用户给 needle PNG，实时截图里找位置**（one-shot TM），但相似度定义要从「像素相关」升级为「**稳定区域结构相似**」。
...
| **推理** | `spotter-plugin-match-learned` + ort |
...
2. **`spotter-plugin-match-ncc`：Masked NCC**（或 weighted NCC）

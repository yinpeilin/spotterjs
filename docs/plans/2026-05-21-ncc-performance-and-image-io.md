...
**架构：** 生产链路保持 `screen → node → core → plugin`；图像 decode/encode 收敛到 `spotterjs-core::image`（基于 `image` crate，纯 Rust、跨平台）；NCC 算法分三阶段优化（并行修复 → 预计算+积分图 → 滑动窗口）。

**技术栈：** Rust (`image` 0.25, `rayon`), `@spotterjs/node` (napi), `@spotterjs/core`
...
`load_rgba_from_*` 实现在 `crates/spotterjs-plugin-match-ncc/src/lib.rs`，由 `spotterjs-core/src/matcher.rs` 调用。
...
### 任务 A1：新建 `spotterjs-core/src/image.rs`

**文件：**
- 创建：`crates/spotterjs-core/src/image.rs`
- 修改：`crates/spotterjs-core/src/lib.rs`（mod + re-export）
- 修改：`crates/spotterjs-core/src/matcher.rs`（改 import）
- 修改：`crates/spotterjs-plugin-match-ncc/src/lib.rs`（删除 load 函数，或 re-export core）

- [ ] **步骤 1：迁移 decode 函数到 core**

```rust
// crates/spotterjs-core/src/image.rs
...
运行：`cargo test -p spotterjs-core image::`
预期：PASS
...
**文件：**
- 修改：`crates/spotterjs-node/src/lib.rs`
...
pub fn load_image_from_path(path: String) -> Result<JsCaptureImage> {
    let img = spotterjs_core::load_rgba_from_path(Path::new(&path))?;
    capture_to_js(&img)
}

#[napi(js_name = "getImageSize")]
pub fn get_image_size(path: String) -> Result<JsSize> {
    let (w, h) = spotterjs_core::image_size_from_path(Path::new(&path))?;
...
- [ ] 在 `spotterjs-core/src/image.rs` 添加 `encode_rgba_to_png`
...
**文件：**
- 修改：`crates/spotterjs-plugin-match-ncc/src/ncc.rs`
- 修改：`crates/spotterjs-plugin-match-ncc/src/multiscale.rs`
...
**文件：** `crates/spotterjs-plugin-match-ncc/src/ncc.rs`
...
**文件：** 新建 `crates/spotterjs-plugin-match-ncc/src/integral.rs`
...
```bash
# 纯匹配基准（Rust path，无截屏）
cargo run -p spotterjs-plugin-match-ncc --example bench_ncc -- test-output/capture.png test-output/needle.png
```
...
**文件：** `crates/spotterjs-plugin-match-ncc/src/multiscale.rs`
...
**文件：** 新建 `crates/spotterjs-plugin-match-ncc/examples/bench_ncc.rs`

...
| `desktop_capture_screen` | Capture screen or region as a workspace PNG file path |
| `desktop_capture_window` | Capture by window id as a workspace PNG file path |
...
Capture tools accept `detail: "high" | "original"`. The default `high` keeps
MCP responses practical by downscaling captures whose long edge exceeds 1600
pixels. Use `original` when OCR, pixel inspection, or template debugging needs
the full-size PNG:
